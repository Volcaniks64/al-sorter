...
options.forEach((opt, index) => {
  if (opt.key === 'roster') {
    if (Array.isArray(optTaken[index])) {
        optTaken[index].forEach((included, subIndex) => {
            const key = opt.sub[subIndex].key;

            if (!included) {
                characterDataToSort = characterDataToSort.filter(char => !char.opts[key]);
            }
        });
    }
  } else if ('sub' in opt) {
...
function getImageFilename(char) {
  return char.img || char.name.replace(/\s+/g, "_") + ".png";
}
...
function setLatestDataset() {
  ...
  populateOptions();
  setRandomStartingImages();
  function setRandomStartingImages() {
  const characters = characterData;

  if (characters.length < 2) return;

  const leftIndex = Math.floor(Math.random() * characters.length);
  ...
  document.querySelector('.left.sort.image').src =
  imageRoot + getImageFilename(characters[leftIndex]);

  document.querySelector('.right.sort.image').src =
  imageRoot + getImageFilename(characters[rightIndex]);
}
}
...
function preloadImages() {
  ...
  return Promise.all(characterDataToSort.map(async (char, idx) => {
    characterDataToSort[idx].img =
      await loadImage(imageRoot + getImageFilename(char));
  }));
}
